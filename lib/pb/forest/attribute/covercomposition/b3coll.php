...
class B3Coll  extends \ContentColl  {
    ...
    public function __construct() {
        parent::__construct(new \forest\attribute\covercomposition\B3());
    }
    /**
    ...
    protected function customSelect(\Zend_Db_Select $select,array $criteria ) {
        ...
        switch (get_class($this->content->getTable()->getAdapter())) {
            case 'Zend_Db_Adapter_Mysqli':
                $select->from('comp_arb',array(
                    '*',
                    'cod_colt_descriz'=>new \Zend_Db_Expr(
                        '( SELECT CONCAT(diz_arbo.nome_itali , \' | \' , diz_arbo.nome_scien) FROM diz_arbo WHERE diz_arbo.cod_coltu=comp_arb.cod_coltu) '
                     )
                ));
                break;
