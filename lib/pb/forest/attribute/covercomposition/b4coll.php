<?php
/**
 * Manages B4 Cover Composition collection
 * 
 * Manages B4 Cover Composition collection
 * 
...
class B4Coll  extends \ContentColl  {
    ...
    public function __construct() {
        parent::__construct(new \forest\attribute\covercomposition\B4());
    }
    /**
     * Sets the form reference
     * @param \forest\entity\b\B4 $form Entity b3
     */
    ...
    protected function customSelect(\Zend_Db_Select $select,array $criteria ) {
        ...
        switch (get_class($this->content->getTable()->getAdapter())) {
            case 'Zend_Db_Adapter_Mysqli':
                $select->from('arboree4b',array(
                    '*',
                    'cod_colt_descriz'=>new \Zend_Db_Expr(
                        '( SELECT CONCAT(diz_arbo.nome_itali , \' | \' , diz_arbo.nome_scien) FROM diz_arbo WHERE diz_arbo.cod_coltu=arboree4b.cod_coltu) '
                     )
                ));
            break;
            case 'Zend_Db_Adapter_Pgsql':
                $select->from('arboree4b',array(
                    '*',
                    'cod_colt_descriz'=>new \Zend_Db_Expr(
                        '( SELECT diz_arbo.nome_itali || \' | \' || diz_arbo.nome_scien FROM diz_arbo WHERE diz_arbo.cod_coltu=arboree4b.cod_coltu) '
                     )
                ));
            break;
        }
