<?php
/**
 * Manages B3 Tree line Composition collection
 * 
 * Manages B3 Tree line Composition collection
 * 
...
class B3TreeLineCompositionColl  extends \ContentColl  {
    ...
    public function __construct() {
        parent::__construct(new B3TreeLineComposition());
    }
    /**
    ...
    protected function customSelect(\Zend_Db_Select $select,array $criteria ) {
        ...
        switch (get_class($this->content->getTable()->getAdapter())) {
            ...
            $select->from('arb_colt',array(
                '*',
                'cod_coltr_descriz'=>new \Zend_Db_Expr(
                    '( SELECT CONCAT(diz_arbo.nome_itali , \' | \' , diz_arbo.nome_scien) FROM diz_arbo WHERE diz_arbo.cod_coltu=arb_colt.cod_coltu) '
                 )
            ));
            break;
