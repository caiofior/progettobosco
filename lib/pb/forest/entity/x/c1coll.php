<?php
/**
 * Manages Entity C1 forest compartment Collection
 * 
 * Manages Entity C1 forest compartment Collection
 * 
 ...
 * @copyright CRA
 */
class C1Coll extends \forest\template\EntityColl {
    /**
    ...
    public function __construct() {
        parent::__construct(new \forest\entity\x\C1());
    }
    /**
     * Set the form C
    ...
    protected function customSelect(\Zend_Db_Select $select,array $criteria ) {
        ...
        if ($this->c instanceof \forest\entity\x\C) {
            $select->where('sched_c1.proprieta = ?', $this->c->getData('proprieta'))
                   ->where('sched_c1.cod_part = ?', $this->c->getData('cod_part'))
                   ->where('sched_c1.cod_fo = ?', $this->c->getData('cod_fo'))
                   ->where('sched_c1.tipo_ril = ?', $this->c->getData('tipo_ril'))
                   ->where('sched_c1.data = ?', $this->c->getData('data'));
        }
        return $select;
    }
     /**
