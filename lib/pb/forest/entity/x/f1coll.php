<?php
/**
 * Manages Entity F1 forest compartment Collection
 * 
 * Manages Entity F1 forest compartment Collection
 * 
 ...
 * @copyright CRA
 */
class F1Coll extends \forest\template\EntityColl {
    /**
    ...
    public function __construct() {
        parent::__construct(new \forest\entity\x\F1());
    }
    /**
     * Set the form F
    ...
    protected function customSelect(\Zend_Db_Select $select,array $criteria ) {
        ...
        if ($this->f instanceof \forest\entity\x\D) {
            $select->where('sched_f1.proprieta = ?', $this->f->getData('proprieta'))
                   ->where('sched_f1.cod_part = ?', $this->f->getData('cod_part'))
                   ->where('sched_f1.cod_fo = ?', $this->f->getData('cod_fo'))
                   ->where('sched_f1.n_camp = ?', $this->f->getData('n_camp'))
                   ->where('sched_f1.tipo_ril = ?', $this->f->getData('tipo_ril'))
                   ->where('sched_f1.data = ?', $this->f->getData('data'));
        }
        return $select;
    }
     /**
